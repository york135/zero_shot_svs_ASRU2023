...
# [0.25, 0.5, 0.75, 1.0, 2.0] sec
possible_time_length = [6000, 12000, 18000, 24000, 48000]
...
class Trainer(object):
    ...
    def _train_step(self, batch):
        ...
        if self.steps > self.config.get("generator_train_start_steps", 0):
            ...
            if self.config["use_stft_loss"]:
                sc_loss, mag_loss, delta_loss = self.criterion["stft"](y_, y)
                gen_loss += sc_loss + mag_loss + delta_loss
                ...
                if self.config["stft_loss_params"]["use_delta"]:
                    self.total_train_loss[
                        "train/log_stft_delta_loss"
                    ] += delta_loss.item()
            ...
            if self.steps > self.config["discriminator_train_start_steps"]:
                # select from [0.25, 0.5, 0.75, 1.0, 2.0]
                select_time = int(torch.rand(1) * 5.0)
                window_clips = possible_time_length[select_time]
                start_choices = 48000 - window_clips

                start = int(torch.rand(1) * start_choices)

                p_ = self.model["discriminator"](y_[:,:,start:start+window_clips])
    ...
    def _eval_step(self, batch):
        ...
        if self.config["use_stft_loss"]:
            sc_loss, mag_loss, delta_loss = self.criterion["stft"](y_, y)
            aux_loss += sc_loss + mag_loss + delta_loss
            self.total_eval_loss["eval/spectral_convergence_loss"] += sc_loss.item()
            self.total_eval_loss["eval/log_stft_magnitude_loss"] += mag_loss.item()
            if self.config["stft_loss_params"]["use_delta"]:
                self.total_eval_loss["eval/log_stft_delta_loss"] += delta_loss.item()
