...
class ResNetSE(nn.Module):
    ...
    def pre_net(self, x):
        fb_result = self.torchfb(x)+1e-6
        if self.log_input: x = torch.log(fb_result)
        out = self.instancenorm(x).unsqueeze(1)
        return out, fb_result
...
if __name__ == "__main__":
    model = MainModel(nOut=512, log_input=True, encoder_type="ASP", initial_model="baseline_lite_ap.model").to(device)